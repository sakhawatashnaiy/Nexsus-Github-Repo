...
export const createMeeting = catchAsync(async (req, res) => {
    ...
    const { inviteeId, title, description, location, startAt, endAt } = req.body;
...
export const listMeetings = catchAsync(async (req, res) => {
    ...
    return sendResponse(res, {
        message: 'OK',
        data: meetings.map((m) => toMeetingDto(m)),
    });
});
export const listMeetingEvents = catchAsync(async (req, res) => {
    const userId = req.user.id;
    const { from, to, status } = req.query;
    const filter = {
        participants: userId,
...
export const acceptMeeting = catchAsync(async (req, res) => {
    const userId = req.user.id;
...
export const rejectMeeting = catchAsync(async (req, res) => {
    const userId = req.user.id;
    const id = req.params.id;
    if (!mongoose.isValidObjectId(id))
        return sendResponse(res, { success: false, message: 'Not found', errors: [] });
...
export const cancelMeeting = catchAsync(async (req, res) => {
    const userId = req.user.id;
    const id = req.params.id;
    if (!mongoose.isValidObjectId(id))
        return sendResponse(res, { success: false, message: 'Not found', errors: [] });
