...
export const listDocuments = catchAsync(async (req, res) => {
    ...
    const filter = { uploadedById: userId, isDeleted: false };
...
export const uploadDocument = catchAsync(async (req, res) => {
    ...
    return res.status(StatusCodes.CREATED).json({
        success: true,
        message: 'Uploaded',
        data: toDto(doc.toObject()),
    });
});
export const uploadNewVersion = catchAsync(async (req, res) => {
    const userId = req.user.id;
    if (!mongoose.isValidObjectId(req.params.id))
        return sendResponse(res, { success: false, message: 'Not found', errors: [] });
    const file = req.file;
    if (!file) {
        return sendResponse(res.status(StatusCodes.UNPROCESSABLE_ENTITY), {
            success: false,
    ...
    await existing.save();
    return sendResponse(res, { message: 'Uploaded', data: toDto(existing.toObject()) });
});
export const updateDocument = catchAsync(async (req, res) => {
    const userId = req.user.id;
    if (!mongoose.isValidObjectId(req.params.id))
        return sendResponse(res, { success: false, message: 'Not found', errors: [] });
    const updated = await DocumentModel.findOneAndUpdate({ _id: req.params.id, uploadedById: userId, isDeleted: false }, { $set: req.body }, { new: true }).lean();
    if (!updated)
        return sendResponse(res, { success: false, message: 'Not found', errors: [] });
...
export const uploadSignature = catchAsync(async (req, res) => {
    ...
    const updated = await DocumentModel.findOneAndUpdate({ _id: req.params.id, uploadedById: userId, isDeleted: false }, {
        $set: {
