...
export const updateProject = catchAsync(async (req, res) => {
    ...
    const project = await ProjectModel.findOneAndUpdate({ _id: req.params.id, ownerId: userId, isDeleted: false }, { $set: req.body }, { new: true });
    if (!project)
        return sendResponse(res, { success: false, message: 'Not found', errors: [] });
    return sendResponse(res, {
        message: 'Updated',
...
export const deleteProject = catchAsync(async (req, res) => {
    ...
    if (!mongoose.isValidObjectId(req.params.id))
        return sendResponse(res, { success: false, message: 'Not found', errors: [] });
    const project = await ProjectModel.findOneAndUpdate({ _id: req.params.id, ownerId: userId, isDeleted: false }, { $set: { isDeleted: true, deletedAt: new Date() } }, { new: true });
    if (!project)
        return sendResponse(res, { success: false, message: 'Not found', errors: [] });
