...
function chooseProvider(requested?: PaymentProvider): PaymentProvider {
  return requested ?? env.PAYMENT_PROVIDER
}

function toDto(t: TransactionDoc & { _id: unknown }) {
  return {
    id: String(t._id),
    type: t.type,
    status: t.status,
...
export const listTransactions = catchAsync(async (req: Request, res: Response) => {
  ...
  const q = req.query as unknown as { page: number; limit: number; status?: string; type?: string }
  ...
  return sendResponse(res, {
    message: 'OK',
    data: items.map((t) => toDto(t as unknown as TransactionDoc)),
...
export const deposit = catchAsync(async (req: Request, res: Response) => {
  ...
  const provider = chooseProvider(req.body?.provider as PaymentProvider | undefined)
  ...
  if (provider === 'stripe' && !env.STRIPE_SECRET_KEY) {
    await TransactionModel.updateOne(
      { _id: tx._id },
      { $set: { status: 'failed', failureReason: 'Stripe not configured' } },
    )
  }
  ...
  return res.status(StatusCodes.CREATED).json({
    success: true,
    message: 'Created',
    data: toDto((await TransactionModel.findById(tx._id).lean()) as unknown as TransactionDoc),
  })
})
