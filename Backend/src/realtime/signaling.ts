...
type SignalingOfferPayload = {
  roomId: string
  sdp: unknown
}

type SignalingAnswerPayload = {
  roomId: string
  sdp: unknown
}

type SignalingIcePayload = {
  roomId: string
  candidate: unknown
}

export function initSignalingServer(httpServer: HttpServer) {
  ...
  io.on('connection', (socket) => {
    socket.on('room:join', async (payload: JoinRoomPayload, ack?: (res: { participants: number; isInitiator: boolean }) => void) => {
    ...
    socket.on('webrtc:offer', (payload: SignalingOfferPayload) => {
      const roomId = payload?.roomId?.trim()
      if (!roomId) return
      socket.to(roomId).emit('webrtc:offer', { from: socket.id, sdp: payload.sdp })
    })

    socket.on('webrtc:answer', (payload: SignalingAnswerPayload) => {
      const roomId = payload?.roomId?.trim()
      if (!roomId) return
      socket.to(roomId).emit('webrtc:answer', { from: socket.id, sdp: payload.sdp })
    })

    socket.on('webrtc:ice-candidate', (payload: SignalingIcePayload) => {
      const roomId = payload?.roomId?.trim()
      if (!roomId) return
